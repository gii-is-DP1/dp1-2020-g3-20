...
public class CocineroControllerTest {
	...
	void setup() {
		...
		given(this.cocineroService.findCamereroById(TEST_COCINERO_ID)).willReturn(cocinero);
	...
	void initDeleteCocinero() throws Exception {
		mockMvc.perform(get("/cocinero/{cocineroId}/delete", 1))
		.andExpect(MockMvcResultMatchers.status().isOk())
		  .andExpect(MockMvcResultMatchers.view().name("redirect:/cocinero/"));
	}
